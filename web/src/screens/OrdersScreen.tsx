    ...
    try {
      ...
      setPageNotice({
        ...
        text: payload.status === 'refunded' || payload.orderDeleted
          ...
          : `退款成功：¥${Number(payload.refundAmount || 0).toFixed(2)}，剩余订单金额 ¥${Number(payload.remainingDiscountAmount || 0).toFixed(2)}`,
      });
      ...
      {refundModalOrderId && (
        <div className="fixed inset-0 bg-black/70 z-[80] flex items-center justify-center p-4 overflow-y-auto">
          <div className="w-full max-w-md max-h-[calc(100vh-2rem)] overflow-y-auto bg-[#121217] border border-white/10 rounded-3xl p-6 space-y-4">
            ...
            <p className="text-sm text-white/60">订单 #{refundModalOrderId.slice(0, 8)} 请选择要退款的商品，已选商品将退款并回库。</p>
            {(() => {
