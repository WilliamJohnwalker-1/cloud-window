...
interface ProductSeriesStore {
  ...
  addSeries: (input: ProductSeriesCreateInput) => Promise<{ error: Error | null }>;
  updateSeries: (id: string, input: ProductSeriesUpdateInput) => Promise<{ error: Error | null }>;
  deleteSeries: (id: string) => Promise<{ error: Error | null }>;
}

export const useProductSeriesStore = create<ProductSeriesStore>()((set) => ({
  series: [],
  isLoading: false,
  error: null,
