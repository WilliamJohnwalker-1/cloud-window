...
### Web v1.3.26 (2026-09-23) - 退货单折扣价修复与详情去重

- 修复退货建单折扣价来源：退货链路改为实时读取 `store_product_prices`（按店铺+商品），优先使用店铺覆盖价，再按店铺折扣率/商品折扣价回退，不再误用缓存导致“直接半价”。
- 修复退货单详情重复行：Web 订单详情在 `return` 单据中不再追加 legacy 合成行，避免同一商品出现一正一负两条明细。
- 历史退货单数据按人工修复流程处理，本次不做历史数据回补。

### Mobile v2.2.22 (2026-09-23) - 退货单折扣价修复

- 修复移动端退货建单折扣价来源：退货链路改为实时读取 `store_product_prices`（按店铺+商品），优先使用店铺覆盖价，再按店铺折扣率/商品折扣价回退，避免误用缓存导致价格偏差。
- 历史退货单数据按人工修复流程处理，本次不做历史数据回补。
...
- 研发模块新增项目置顶能力（最多2个）并置顶优先排序；支持按当前阶段筛选（全阶段）；回退阶段后保持编辑弹窗打开，便于继续补充备注。
...
- Web 侧边栏菜单区改为纵向滚动容器，修复低高度机型下菜单显示不全问题。
- 研发页新增点击筛选（全部/进行中/临近/逾期），支持按时效快速聚焦项目。
- 研发项目“已上架”编辑区将关联字段升级为“商品标识（ID 或 EAN-13）”。
- 新增到货监控面板：按绑定商品聚合展示关联进货单数量、到货数量进度及状态（未下进货单/到货中/已全部到货），确认到货操作仍在进货单流程执行。

### Mobile v2.2.10 (2026-08-13) - 研发列表压缩与到货进度查看

- 移动端研发页新增点击筛选（全部/进行中/临近/逾期），与 Web 筛选口径对齐。
- 研发列表卡片压缩为“仅显示项目名称”的高密度视图，提升同屏可见项目数。
- 研发项目“已上架”编辑区支持输入商品 ID 或 EAN-13 进行绑定匹配。
- 新增到货监控面板，按绑定商品展示关联进货单与到货进度，便于管理员追踪采购状态。
