...
- 退款后订单保留审计：全额退款不再删单，改为保留订单并更新金额/支付状态，避免“已退款但订单消失”影响追溯。
- 部分退款可视化：订单详情新增“已退款商品明细”（商品、数量、退款单价、退款时间），并在后端事件中写入对应退款商品快照。
- 退款后置回退增强：当原子退款同步异常时，Worker 自动回退库存、更新订单行与订单总额，确保营收统计口径即时正确。
- 渠道受理即本地落账：即使网关首轮返回 `pending/405`，只要退款请求被渠道受理，系统也会立即同步退款商品、库存与订单支付状态（`partial_refunded/refunded`），避免“退款到账但状态不变”。
