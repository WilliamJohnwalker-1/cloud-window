-- Migration v8.4: purchase order total cost + one finance record per purchase order

-- 1) purchase_orders add total_cost_amount
ALTER TABLE public.purchase_orders
ADD COLUMN IF NOT EXISTS total_cost_amount NUMERIC(12, 2) NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.purchase_orders.total_cost_amount IS 'Purchase order total procurement cost snapshot (sum of item ordered_quantity * unit_cost at creation)';

-- Backfill historical totals from purchase_order_items
WITH purchase_totals AS (
  SELECT
    poi.purchase_order_id,
    ROUND(SUM(COALESCE(poi.ordered_quantity, 0)::NUMERIC * COALESCE(poi.unit_cost, 0)), 2) AS total_cost_amount
  FROM public.purchase_order_items poi
  GROUP BY poi.purchase_order_id
)
UPDATE public.purchase_orders po
SET total_cost_amount = purchase_totals.total_cost_amount,
    updated_at = NOW()
FROM purchase_totals
WHERE po.id = purchase_totals.purchase_order_id
  AND (
    po.total_cost_amount IS NULL
    OR ABS(COALESCE(po.total_cost_amount, 0) - COALESCE(purchase_totals.total_cost_amount, 0)) > 0.009
  );

UPDATE public.purchase_orders
SET total_cost_amount = 0
WHERE total_cost_amount IS NULL;

-- 2) create_purchase_order_v2: write purchase_orders.total_cost_amount
...
-- 3) confirm_purchase_item_delivery: aggregate purchase finance into one row per purchase_order
...
DECLARE
  ...
  v_existing_transaction_amount NUMERIC(12, 2);
...
BEGIN
  ...
  INTO v_transaction_id, v_existing_transaction_amount
  ...
  ELSE
    ...
    SET amount = ROUND(COALESCE(v_existing_transaction_amount, 0) + v_purchase_amount, 2),
