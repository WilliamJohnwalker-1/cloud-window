-- Migration v5.4: Update distribution order quantity rules in create_batch_order_atomic
-- Execute after migrate-v5.3-product-store-fields.sql
--
-- Rule change:
-- 1) Distributor: each non-sample item quantity must be >= 30
-- 2) Admin / super_admin: quantity > 0 (already enforced globally)
-- 3) Sample lines remain exempt from distributor minimum rule
-- 4) Retail/settlement flows are unaffected (this migration only replaces create_batch_order_atomic)
